      ...
      <div style={{ marginTop: 15, position: 'relative' }}>
        <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <img src={dados.autorFoto || "https://via.placeholder.com/40"} alt="user" style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover' }} />
            ...
            <div style={{ flex: 1 }}>
                ...
                {respondendoA === dados.id && (
                    <div className="animeLeft" style={{ marginTop: 10, marginBottom: 10, display: 'flex', gap: 5 }}>
                         ...
                         <div style={{ flex: 1 }}>
                            <textarea autoFocus value={textoResposta} onChange={(e) => setTextoResposta(e.target.value)} placeholder={`Replying to ${dados.autorNome}...`} style={{ width: '100%', background: '#272729', border: '1px solid #343536', color: 'white', padding: 8, borderRadius: 4, minHeight: 60 }} />
...
export default function Comentarios({ targetId, targetType = 'capitulo' }) {
  ...
  useEffect(() => {
    const q = query(collection(db, "comentarios"), where("targetId", "==", targetId), orderBy("data", "asc"));
  ...
  async function handleEnviar(parentId = null) {
    ...
    try {
      await addDoc(collection(db, "comentarios"), {
        texto: textoFinal, autorId: user.uid, autorNome: user.name, autorFoto: user.avatar, targetId: targetId, targetType: targetType, parentId: parentId, likes: [], data: serverTimestamp()
      });
      if (parentId) { setRespondendoA(null); setTextoResposta(''); } else { setNovoTexto(''); }
    } catch (error) { console.error(error); alert("Error sending comment."); }
  }
